...
**Company lens (independent):** CoreWeave (`coreweave`)  
**Innovation:** Rack Power Autopilot  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim CoreWeave affiliation, proprietary access, or production deployment.

### Bottleneck
Power, hardware reliability, scheduling, checkpointing, recovery, and utilization at cluster scale.

### Brick wall
Maintaining economic and operational reliability amid component failures and huge infrastructure expansion.

### Mechanism to implement
Close the loop from power/thermal telemetry to job admission decisions with hard refuse states.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply CoreWeave employment, endorsement, or proprietary systems access.
...
File: `src/rack_power_autopilot.py`

- Expand `RackPowerAutopilot` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_rack_power_autopilot.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_rack_power_autopilot.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary CoreWeave APIs
